...
function Row({row, ponovoPreuzmi}) {
    function otkazi(){
        axios
          .delete(`${Putanje.knjigeGWURL}/pretplata/${row.Id}`)
          .then((response) => {
...
export default function PretplateTabela() {
    const [pretplate, setPretplate] = useState([])

    const { items, requestSort, sortConfig } = useSortableData(pretplate);
    ...
    const preuzmiSve = async () => {
        axios
          .get(Putanje.knjigeGWURL+ "/pretplata/" + AuthServis.preuzmiKorisnika().Id)
          .then((response) => {
            console.log(response.data);
            setPretplate(response.data)
